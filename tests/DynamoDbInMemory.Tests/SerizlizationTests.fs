...
open DynamoDbInMemory.Serialization
...
type SerializationTests(output: ITestOutputHelper) =
    ...
    static let replicate (client: IInMemoryDynamoDbClient) regionName tableName = function
        | ChangeType.Update ->
    ...
    let ``DeSerialize distributed database, preserves replication`` ``insert new replicat at top`` =
        ...
        task {
            ...
            use client1 = InMemoryDynamoDbClient.Create(db.GetDatabase (ValueSome logger) { regionId = "eu-north-1" }, logger)
            use client2 = InMemoryDynamoDbClient.Create(db.GetDatabase (ValueSome logger) { regionId = "eu-west-1" }, logger)
            use client3 = InMemoryDynamoDbClient.Create(db.GetDatabase (ValueSome logger) { regionId = "somewhere-else" }, logger)
