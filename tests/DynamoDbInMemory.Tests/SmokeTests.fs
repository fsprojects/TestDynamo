...
type SmokeTests(output: ITestOutputHelper) =
    ...
    let ``List tables smoke test`` () =
        ...
        task {
            use client = InMemoryDynamoDbClient.Create()
    ...
    let ``List global tables smoke test`` () =
        ...
        task {
            ...
            use client = InMemoryDynamoDbClient.Create(host, {regionId = "r1" })
    ...
    let ``Create global table, describe global table smoke test`` () =
        ...
        task {
            ...
            use client = InMemoryDynamoDbClient.Create(commonHost, {regionId = "r1" })
