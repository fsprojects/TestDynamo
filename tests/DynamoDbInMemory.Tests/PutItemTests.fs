...
type PutItemTests(output: ITestOutputHelper) =
    ...
    let maybeBatch batch (client: IInMemoryDynamoDbClient) (req: PutItemRequest) =
        if batch
    ...
    [<Theory>]
    [<ClassData(typedefof<TwoFlags>)>]
    let ``Put item, with various condition expression scenarios`` ``item exists`` ``condition matches`` =
    ...
    let ``Batch write item, invalid region, throws`` distributed ``invalid region``: Task<unit> =
        ...
        task {
            ...
            use client =
                if distributed then InMemoryDynamoDbClient.Create(dHost, host.Id)
                else
                    InMemoryDynamoDbClient.Create(cloneHost writer)
            let req =
    ...
    let ``Batch write item, one put, one delete, executes both`` (): Task<unit> =
        ...
        task {
            ...
            let! tables = sharedTestData ValueNone // (ValueSome output)
            use host = cloneHost writer
            use client = InMemoryDynamoDbClient.Create(host)
            let tableName = (Tables.get true true tables).name
            ...
            let fromKeys pk sk =
                Map.add "TablePk" (DynamoDbInMemory.Model.AttributeValue.String pk) Map.empty
                |> Map.add "TableSk" (DynamoDbInMemory.Model.AttributeValue.Number sk)

            let put = WriteRequest()
            put.PutRequest <- PutRequest()
            put.PutRequest.Item <-
                putItem
                |> Map.add "Blabla" (DynamoDbInMemory.Model.AttributeValue.Number 3344M)  
                |> itemToDynamoDb
