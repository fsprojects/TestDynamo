...
type DatabaseReplicationTests(output: ITestOutputHelper) =
    ...
    static let rec replicate' (output: ITestOutputHelper, random) (client: IInMemoryDynamoDbClient) regionName tableName changeType =
    ...
    static let rec replicateWithLevel (output: ITestOutputHelper, random, logLevel) (host: DistributedDatabase, dbId) regionName tableName changeType changeFromDefaultBehaviour  =
        ...
        task {
            ...
            use client = InMemoryDynamoDbClient.Create(host, dbId, logger)
    ...
    let deReplicate (client: IInMemoryDynamoDbClient) regionName tableName = function
        | ChangeType.Update ->
            let req = UpdateTableRequest()
            req.TableName <- tableName
            req.ReplicaUpdates <- System.Collections.Generic.List()
    ...
    let setUp2RegionsWithCustomSettings settings logger doReplication =
        task {
            ...
            let client1 = InMemoryDynamoDbClient.Create(host, { regionId = "eu-west-1"})
            let client2 = InMemoryDynamoDbClient.Create(host, { regionId = "eu-north-1"})
    ...
    let put tableName item (client: IInMemoryDynamoDbClient) =
        client.PutItemAsync(tableName, itemToDynamoDb item)
        |> Io.ignoreTask

    let assertItem tableName keys expected (client: IInMemoryDynamoDbClient) =
        task {
    ...
    let assertNoItem tableName keys (client: IInMemoryDynamoDbClient) =
        task {
            let! actual = client.GetItemAsync(tableName, itemToDynamoDb keys)
    ...
    let executePutSmokeTest direction item itemKeys struct (table: DynamoDbInMemory.Tests.TableDescription, host: DistributedDatabase, client1: IInMemoryDynamoDbClient, client2: IInMemoryDynamoDbClient, _) =
    ...
    let ``Replication smoke tests, DELETE`` direction =
        ...
        task {
            ...
            let assertExists (client: IInMemoryDynamoDbClient) exists =
                task {
    ...
    let ``Replication smoke tests, Competing PUTS`` dominantDirection =
        ...
        task {
            ...
            client1.ProcessingDelay <- TimeSpan.Zero
            client2.ProcessingDelay <- TimeSpan.Zero
    ...
    let ``Replication smoke tests, Competing PUT and DELETE`` parentDominant putDominant itemExists =
        ...
        task {
            ...
            let doPut (client: IInMemoryDynamoDbClient) = client.PutItemAsync(table.name, itemToDynamoDb value) |> Io.ignoreTask
            let doDelete (client: IInMemoryDynamoDbClient) = client.DeleteItemAsync(table.name, itemToDynamoDb keys) |> Io.ignoreTask
    ...
    let ``Replicate DB, 4 regions, Operations propagate correctly`` ``wait for each operation`` synchronous =
        ...
        task {
            ...
            use client1 = InMemoryDynamoDbClient.Create(host, { regionId = "eu-west-1"})
            use client2 = InMemoryDynamoDbClient.Create(host, { regionId = "eu-north-1"})
            use client3 = InMemoryDynamoDbClient.Create(host, { regionId = "ap-south-2"})
            use client4 = InMemoryDynamoDbClient.Create(host, { regionId = "us-east-2"})
            let! tables = sharedTestData ValueNone
            let table = Tables.getByStreamsEnabled true tables
            ...
            // assert
            let scan (client: IInMemoryDynamoDbClient) =
                client.ScanAsync(
                    ScanBuilder.empty
                    |> ScanBuilder.withTableName table.name
                    |> ScanBuilder.req)
    ...
    let ``Distributed DB, some random disposal tests`` () =
        ...
        task {
            ...
            do! replicate (output, random) (host, "eu-west-1") "eu-north-1" "Tabbb1" ValueNone (ValueSome subscriberSettings)
    ...
    let ``Replicate DB, failure scenarios, fails and does not replicate`` ``table exists already`` ``no streams enabled`` ``has invalid table name`` ``invalid region`` =
        ...
        task {
            ...
            use client1 = InMemoryDynamoDbClient.Create(host, { regionId = region1})
            use client2 = InMemoryDynamoDbClient.Create(host, { regionId = region2})
            let tableName = "Tab1"            
            let createTable (client: IInMemoryDynamoDbClient) =
                let req =
                    TableBuilder.empty
            ...
            do! client1.AwaitAllSubscribers CancellationToken.None
    ...
    let put (client: IInMemoryDynamoDbClient) tableName pk =
        let item =
            ItemBuilder.empty
            |> ItemBuilder.withTableName tableName
    ...
    let ``Create host with replications`` region1 region2 tableName writer streamSubscriberOptions =
        ...
        task {
            ...
            do! client1.AwaitAllSubscribers CancellationToken.None
            do! client2.AwaitAllSubscribers CancellationToken.None
    ...
    let ``Delete replication, behaves correctly`` () =
        ...
        task {
            ...
            let assertItem (client: IInMemoryDynamoDbClient) pk =
    ...
    let ``Clone host with replicas, behaves correctly`` () =
        ...
        task {
            ...
            use client1 = InMemoryDynamoDbClient.Create(host2, { regionId = region1})
            use client2 = InMemoryDynamoDbClient.Create(host2, { regionId = region2})
            let! _ = put client1 tableName "v3"
            let! _ = put client2 tableName "v4"
    ...
    let ``Host with replicas, delete a table, still works`` ``delete primary`` =
        ...
        task {
            ...
            use deleteClient = InMemoryDynamoDbClient.Create(host, { regionId = delete})
            use keepClient = InMemoryDynamoDbClient.Create(host, { regionId = keep})
            let! _ = deleteClient.DeleteTableAsync(tableName)
            do! deleteClient.AwaitAllSubscribers CancellationToken.None
    ...
    let ``Update global table, create index, new index conflicts with existing data, fails`` ``index creator`` ``fix type`` =
        ...
        task {
            ...
            use parentClient = InMemoryDynamoDbClient.Create(host, parentDbId, (writer :> Microsoft.Extensions.Logging.ILogger))
            use childClient = InMemoryDynamoDbClient.Create(host, childDbId, (writer :> Microsoft.Extensions.Logging.ILogger))
            parentClient.ProcessingDelay <- TimeSpan.Zero
            childClient.ProcessingDelay <- TimeSpan.Zero
    ...
    let ``Write action with failed condition, does not replicate`` action =
        ...
        let put pk sk table (client: IInMemoryDynamoDbClient) =
            ItemBuilder.empty
        ...
        let delete pk sk table (client: IInMemoryDynamoDbClient) =
            ItemBuilder.empty
            |> ItemBuilder.withTableName table
            |> ItemBuilder.withAttribute "TablePk" "S" pk
            |> (sk ?|> (ItemBuilder.withAttribute "TableSk" "N") ?|? id)
        ...
        let update pk sk table (client: IInMemoryDynamoDbClient) =
    ...
    let ``Replication, load testing`` () =
        ...
        task {
          ...
          //  use host = new DistributedDatabase(writer) // There are a lot of logs
            ...
            let put (client: IInMemoryDynamoDbClient) pk =
                let item =
                    ItemBuilder.empty
                    |> ItemBuilder.withTableName tableName
                    |> ItemBuilder.withAttribute "TablePk" "S" pk
            ...
            let buildReplicatedClient path =
                ...
                | [] -> invalidOp ""
                | [x] ->
                    struct (x, InMemoryDynamoDbClient.Create(host, { regionId = x}))
                    |> Task.FromResult
                | head::tail ->
                    let parentName = List.rev tail |> Str.join "-"
                    let name = $"{parentName}-{head}"
                    let client = InMemoryDynamoDbClient.Create(host, { regionId = name})
                    task {
                        use parent = InMemoryDynamoDbClient.Create(host, { regionId = parentName})
                        parent.ProcessingDelay <- TimeSpan.Zero
