...
type TransactWriteItemTests(output: ITestOutputHelper) =
        ...
        task {
            ...
            let client =
                ...
                |> ValueOption.defaultWith (fun _ ->
                    ...
                    InMemoryDynamoDbClient.Create(host, writer))
    ...
    let getFromWriteReq (client: IInMemoryDynamoDbClient) (req: TransactWriteItemsRequest) =
        task {
    ...
    let buildReq (client: IInMemoryDynamoDbClient) tableName f =
    ...
    let writeAndGet req (client: IInMemoryDynamoDbClient) tableName =
        task {
    ...
    let setUpglobalTable() =
        task {
            ...
            let _ = host
            let client1 = InMemoryDynamoDbClient.Create(host, fromRegion, writer)
            let client2 = InMemoryDynamoDbClient.Create(host, toRegion, writer)
