...
type GetItemTests(output: ITestOutputHelper) =
    ...
    let executeAsBatchGet (client: IInMemoryDynamoDbClient) (req: GetItemRequest) =
        let kAndAttr =
    ...
    let executeAsTransactGet (client: IInMemoryDynamoDbClient) (req: GetItemRequest) =
        let kAndAttr =
    ...
    let ``Batch get item, variable request count, maybe throws`` ``req count`` ``success`` =
        ...
        task {
            ...
            // arrange
            let keys hasSk (table: DynamoDbInMemory.Tests.TableDescription) =
    ...
    let ``Batch get item, two tables in different regions, returns item`` (): Task<unit> =
        ...
        task {
            ...
            use host = cloneHost writer
            use dHost = new DistributedDatabase(host.BuildCloneData())
            use client = InMemoryDynamoDbClient.Create(dHost, host.Id)
            let table = Tables.get true true tables
    ...
    let ``Batch get item, multiple pages, returns correct items`` ``consistant read``: Task<unit> =
        ...
        let rec executeUntilDone (client: IInMemoryDynamoDbClient) acc (req: Dictionary<string,KeysAndAttributes>) =
            task {
