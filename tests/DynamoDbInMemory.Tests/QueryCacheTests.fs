...
type QueryCacheTests(output: ITestOutputHelper) =
    ...
    let ``Big cache test: 1. create table; 2. do query+filter+projection; 3. do different query; 4. do different filter; 5. do different projection; 6. alter table and do query`` () =
        ...
        let execute (f: IInMemoryDynamoDbClient -> Task) =
            task {
                use recorder = new TestLogger(output)
                use client = InMemoryDynamoDbClient.Create(commonHost, recorder)
        ...
        task {
            ...
            let setupClient = InMemoryDynamoDbClient.Create(commonHost, setupRecorder)
            // Arrange
            ...
            let baseQuery (client: IInMemoryDynamoDbClient) = 
                baseReq
                |> QueryBuilder.queryRequest
                |> client.QueryAsync
                |> Io.ignoreTask

            let newIndexQuery (client: IInMemoryDynamoDbClient) = 
                baseReq
            ...
            let keyCondChangedQuery (client: IInMemoryDynamoDbClient) = 
                baseReq
            ...
            let projectionExprChangedQuery (client: IInMemoryDynamoDbClient) = 
                baseReq
            ...
            let keyfilterChangedQuery (client: IInMemoryDynamoDbClient) = 
                baseReq 
