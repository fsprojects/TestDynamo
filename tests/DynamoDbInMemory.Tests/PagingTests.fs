...
type PagingTests(output: ITestOutputHelper) =
    ...
    static let sharedTestData =
        ...
        let rec put table (client: IInMemoryDynamoDbClient): int -> Task = function
        ...
        let execution =
            task {
                ...
                use client = InMemoryDynamoDbClient.Create(host, (writer :> Microsoft.Extensions.Logging.ILogger))
    ...
    let rec executePagedQuery (client: IInMemoryDynamoDbClient) req: Task<struct (Dictionary<_, _> list list * int)> =
        task {
    ...
    let rec executePagedScan (client: IInMemoryDynamoDbClient) req: Task<struct (Dictionary<_, _> list list * int)> =
        task {
    ...
    let ``Query and scan on packed index, with min paging + 1, returns 1 page`` opType limit maxScanItems maxPageSizeBytes  =
        ...
        let singleItemSize tableName (client: IInMemoryDynamoDbClient) =
