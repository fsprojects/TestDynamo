...
type PutItemTests(output: ITestOutputHelper) =
    ...
    let ``Put item, with various condition expression scenarios`` ``item exists`` ``condition matches`` ``use legacy inputs`` =
        ...
        task {
            ...
            let client = TestDynamoClient.createClient<AmazonDynamoDBClient> ValueNone true ValueNone true (ValueSome host)
    ...
    let ``Batch write item, invalid region, throws`` ``global`` ``invalid region``: Task<unit> =
        ...
        task {
            ...
            use client =
                if ``global``
                then TestDynamoClient.createGlobalClient ValueNone true (ValueSome host.Id) ValueNone false (ValueSome dHost)
