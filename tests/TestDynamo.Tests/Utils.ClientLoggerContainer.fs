...
/// <summary>A wrapper around an ITestDynamoClient and TestLogger which disposes of both</summary>
type ClientContainer(client: ITestDynamoClient, logger: ILogger) =
    ...
    new(host: Database, logger: ILogger) = new ClientContainer(TestDynamoClient.Create(host, logger), logger)
