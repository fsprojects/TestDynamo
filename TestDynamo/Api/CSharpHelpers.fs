...
type ListBuilder private(itemsReversed: AttributeValue list voption) =
    ...
    member this.AsAttributeList() = itemsReversed ?|? [] |> Seq.rev |> Array.ofSeq |> CompressedList |> AttributeValue.createAttributeList

    member this.AppendNull() = ListBuilder((AttributeValue.createNull())::(itemsReversed  ?|? []))

    member this.Append(value: string) = ListBuilder((AttributeValue.createString value)::(itemsReversed  ?|? []))

    member this.Append(value: decimal) = ListBuilder((AttributeValue.createNumber value)::(itemsReversed  ?|? []))

    member this.Append(value: bool) = ListBuilder((AttributeValue.createBoolean value)::(itemsReversed  ?|? []))

    member this.Append(value: byte array) =
        ListBuilder((AttributeValue.createBinary (Array.copy value))::(itemsReversed  ?|? []))
    ...
    member this.AppendSet(setValues: string seq) =
        let setValues = Seq.map (AttributeValue.createString) setValues |> AttributeSet.create |> AttributeValue.createHashSet
        ListBuilder(setValues::(itemsReversed  ?|? []))

    member this.AppendSet(setValues: decimal seq) =
        let setValues = Seq.map (AttributeValue.createNumber) setValues |> AttributeSet.create |> AttributeValue.createHashSet
        ListBuilder(setValues::(itemsReversed  ?|? []))

    member this.AppendSet(setValues: byte array seq) =
        let setValues = Seq.map (Array.copy >> AttributeValue.createBinary) setValues |> AttributeSet.create |> AttributeValue.createHashSet
        ListBuilder(setValues::(itemsReversed  ?|? []))
...
and
    ...
    MapBuilder private(item: Map<string, AttributeValue> voption) =
        ...
        member this.AsMap() = item ?|? Map.empty |> AttributeValue.createHashMap   

        member this.Null(name: string) = MapBuilder(Map.add name (AttributeValue.createNull()) (item  ?|? Map.empty))

        member this.Attribute(name: string, value: string) = MapBuilder(Map.add name (AttributeValue.createString value) (item  ?|? Map.empty))

        member this.Attribute(name: string, value: decimal) = MapBuilder(Map.add name (AttributeValue.createNumber value) (item  ?|? Map.empty))

        member this.Attribute(name: string, value: byte array) =
            MapBuilder(Map.add name (AttributeValue.createBinary (Array.copy value)) (item  ?|? Map.empty))

        member this.Attribute(name: string, value: bool) = MapBuilder(Map.add name (AttributeValue.createBoolean value) (item  ?|? Map.empty))
