module rec DynamoDbInMemory.Model.Compiler.ReservedWords

open DynamoDbInMemory.Utils

// This and other recursive references to the object(s) being defined will be checked
// for initialization-soundness at runtime through the use of a delayed reference....
// Take the tiny perf his so that the giant list has no code below it 
#nowarn "40"

open System

// using mutable set because F# Set has no custom comparer
type SSet = System.Collections.Generic.HashSet<string>

let private isReservedWord' = lazy(
    let set = SSet(StringComparer.OrdinalIgnoreCase)

    words'
    |> Seq.fold (fun (s: SSet) x ->
        s.Add(x) |> ignoreTyped<bool>
        s) set
    |> ignore

    set.Contains)

let isReservedWord w = isReservedWord'.Value w
