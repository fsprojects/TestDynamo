...
type private DistributedDatabaseData =
    { databases: Map<DatabaseId, DynamoDbInMemory.Api.Database>
...
module private CreateReplication =
    ...
    let processItemBuffer tableName handleError (destinationDb: DynamoDbInMemory.Api.Database) disposer clonedDbVersion =
    ...
    let createReplication
        ...
        struct (left: DynamoDbInMemory.Api.Database, struct (right: DynamoDbInMemory.Api.Database, dbs)): struct (ReplicationDisposalBuilder * DistributedDatabaseState) =
...
module DistributedDatabaseState =
    ...
    let newDatabase id _ = new DynamoDbInMemory.Api.Database(databaseId = id)
    ...
    let getCloneData =
        fun logger -> unwrap >> fun state ->
            let dbs = Seq.map (fun (v: DynamoDbInMemory.Api.Database) -> v.BuildCloneData()) state.databases.Values
            let repKeys =
    ...
    let private ensureDatabase' defaultLogger databaseId logger =
        unwrap >> fun state ->
            ...
            | ValueNone ->
                ...
                let db =
                    ValueOption.map (fun x -> new DynamoDbInMemory.Api.Database(
    ...
    let build iLogger (initialDatabases: Api.DatabaseCloneData list) =
        ...
        let buildDb c =
            ...
            match iLogger with
            | ValueSome l -> new DynamoDbInMemory.Api.Database(l, c)
            | ValueNone -> new DynamoDbInMemory.Api.Database(cloneData = c)
